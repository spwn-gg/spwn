...
Each Claude session works on its own `cm/…` branch, and every turn's work is committed on
...
spwn merges the session's `cm/…` branch into its **base branch** — the branch it was
...
Deleting a session removes **both its worktree and its `cm/…` branch**, so they don't
...
are no `cm/…` branches to merge. To get per-session branches and merging, make the
